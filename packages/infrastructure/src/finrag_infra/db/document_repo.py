...
class PostgresDocumentRepository(AbstractDocumentRepository):
    ...
    async def list_by_owner(self, owner_id: uuid.UUID, limit: int, offset: int) -> list[Document]:
        async with self._sf() as session:
    ...
    async def list_all(self, limit: int, offset: int) -> list[Document]:
        async with self._sf() as session:
            result = await session.execute(
                select(DocumentORM)
                .order_by(DocumentORM.created_at.desc())
    ...
    async def delete(self, document_id: uuid.UUID) -> bool:
        async with self._sf() as session:
            result = await session.execute(delete(DocumentORM).where(DocumentORM.id == document_id))
            await session.commit()
    ...
    async def count_all(self) -> int:
        async with self._sf() as session:
            result = await session.execute(select(func.count()).select_from(DocumentORM))
            return result.scalar_one()
