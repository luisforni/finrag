...
class ChromaVectorStore(AbstractVectorStore):
    ...
    async def add_chunks(
        self, document_id: uuid.UUID, document_filename: str, chunks: list[str]
    ) -> int:
    ...
    async def similarity_search(
        self, query: str, top_k: int, document_ids: list[uuid.UUID] | None = None
    ) -> list[SourceChunk]:
        collection = await self._get_collection()
        where = None
        if document_ids:
            where = (
                ...
                else {"document_id": {"$in": [str(d) for d in document_ids]}}
            )
    ...
    async def delete_document(self, document_id: uuid.UUID) -> bool:
        collection = await self._get_collection()
        results = await collection.get(where={"document_id": str(document_id)}, include=[])
        ids = results.get("ids", [])
        if ids:
